...
# ==========================================
# 🛑 सेटिंग्स (लोकल और रेंडर दोनों सपोर्ट) 🛑
# ==========================================
...
def init_db():
  with get_db() as conn:
    ...
    cursor.execute("""CREATE TABLE IF NOT EXISTS courses (course_id TEXT PRIMARY KEY, promo_media TEXT, amount TEXT, custom_caption TEXT, secret_text TEXT)""")
...
def generate_upi_qr(amount, order_id):
  ...
  draw.rounded_rectangle([box_x - 2, box_y - 2, box_x + box_w + 2, box_y + box_h + 2], radius=8, fill="#0b1329")
  draw.rounded_rectangle([box_x, box_y, box_x + box_w, box_y + box_h], radius=6, fill="#ffffff")
  ...
  draw.polygon([(arrow_x, arrow_y), (arrow_x + arrow_w, arrow_y + (arrow_h // 2)), (arrow_x, arrow_y + arrow_h), (arrow_x + int(arrow_w * 0.4), arrow_y + (arrow_h // 2))], fill="#097939")
  ox = arrow_x + int(arrow_w * 0.6)
  draw.polygon([(ox, arrow_y), (ox + arrow_w, arrow_y + (arrow_h // 2)), (ox, arrow_y + arrow_h), (ox + int(arrow_w * 0.4), arrow_y + (arrow_h // 2))], fill="#F37021")
  draw.rectangle([arrow_x + arrow_w * 2 + 4, arrow_y + 2, arrow_x + arrow_w * 2 + 28, arrow_y + arrow_h - 2], fill="#111827")
  
  strip_y = box_y + int(box_h * 0.68)
  draw.line([(box_x + 6, strip_y), (box_x + box_w - 6, strip_y)], fill="#e2e8f0", width=1)
  draw.ellipse([box_x + int(box_w * 0.20), strip_y + 3, box_x + int(box_w * 0.20) + 6, strip_y + 9], fill="#1a73e8")
  draw.ellipse([box_x + int(box_w * 0.48), strip_y + 3, box_x + int(box_w * 0.48) + 6, strip_y + 9], fill="#5f259f")
  draw.ellipse([box_x + int(box_w * 0.76), strip_y + 3, box_x + int(box_w * 0.76) + 6, strip_y + 9], fill="#00b9f5")
...
def expire_verification(user_id, checking_msg_id, admin_msg_id, course_id):
  if user_id in pending_verifications and pending_verifications[user_id].get("course_id") == course_id:
    ...
    if CHAT_LINK: markup.row(InlineKeyboardButton("💬 डायरेक्ट संपर्क करें", url=CHAT_LINK))
    try:
      bot.send_message(user_id, "⏳ **समय समाप्त!**\nएडमिन की तरफ से 5 मिनट में अप्रूवल नहीं मिला है। कृपया सीधे चैट पर संपर्क करें:", reply_markup=markup, parse_mode="Markdown")
    except: pass
    del pending_verifications[user_id]


# ==========================================
# 🛑 मास्टर कोर्स डिलीवरी (EXACT OKBOT.PY LOGIC) 🛑
# ==========================================
def send_course_to_user(chat_id, course):
  ...
  if len(promo_items) == 1:
    item = promo_items[0]
    # Fallback to normal text if Markdown crashes due to dots/symbols
    try:
        ...
        elif item["type"] == "text": bot.send_message(chat_id, item.get("caption", ""), reply_markup=markup, parse_mode="Markdown")
    except:
        if item["type"] == "photo": bot.send_photo(chat_id, item["file_id"], caption=item.get("caption", ""), reply_markup=markup)
        elif item["type"] == "video": bot.send_video(chat_id, item["file_id"], caption=item.get("caption", ""), reply_markup=markup)
        elif item["type"] == "text": bot.send_message(chat_id, item.get("caption", ""), reply_markup=markup)
  else:
    media_group = []
    ...
    # अलग से टेक्स्ट मैसेज और पेमेंट्स के ऑप्शन 
    try:
...
def handle_all_messages(message):
  ...
  if user_id == ADMIN_ID and user_id in admin_data:
    ...
    if step == "TITLE":
      ...
      bot.send_message(ADMIN_ID, f"✅ बैच टाइटल सेव: **{admin_data[ADMIN_ID]['title']}**\n\n📝 **पहले कोर्स का प्रोमो मीडिया भेजें (फोटो/वीडियो)।**\n(भेजने के बाद नीचे 'Next' बटन दबाएं)", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup().row(InlineKeyboardButton("➡️ Next Step", callback_data="next_price")))
      return
      ...
      if not clean_amt:
        bot.send_message(ADMIN_ID, "❌ कृपया केवल संख्या में कीमत लिखें (उदा: 199 या 499)।")
        return
      ...
      bot.send_message(ADMIN_ID, f"✅ **कीमत ₹{clean_amt} सेव!**\n\n📝 कोई अतिरिक्त कैप्शन या पेमेंट नोट लिखना है तो टाइप करें, वरना 'Skip' दबाएं。", reply_markup=markup)
      return
      ...
      if mode == "single":
        ...
        markup = InlineKeyboardMarkup().row(InlineKeyboardButton("➕ Add Another Course", callback_data="batch_add_next")).row(InlineKeyboardButton("✅ Finish Batch", callback_data="batch_finish"))
  ...
  if user_id in user_states and (message.photo or message.text):
    ...
    check_msg = bot.send_message(user_id, "⏳ आपका पेमेंट प्रूफ मिल गया है। 5 मिनट में अप्रूव हो जाएगा...")
    markup = InlineKeyboardMarkup().row(InlineKeyboardButton("✅ Approve", callback_data=f"app_{user_id}_{course_id}"), InlineKeyboardButton("❌ Deny", callback_data=f"den_{user_id}_{course_id}"))
    ...
    try:
      if message.photo:
        ...
        admin_msg = bot.send_message(DB_CHANNEL_ID, admin_text, reply_markup=markup, parse_mode="Markdown")

      pending_verifications[user_id] = {"checking_msg_id": check_msg.message_id, "admin_msg_id": admin_msg.message_id, "course_id": course_id}
...
def handle_buttons(call):
  ...
  if data.startswith("pay_upi_"):
    ...
    if course:
      ...
      invoice_text = f"👤 **User:** {first_name} {username}\n🆔 **Order ID:** `{order_id}`\n📅 **Date:** {datetime.now().strftime('%d-%m-%Y %I:%M %p')}\n💰 **Amount:** ₹{clean_amt}\n💳 **UPI ID (Tap to Copy):** `{UPI_ID}`\n"
      ...
      try:
          sent_msg = bot.send_photo(chat_id, photo=qr_img_bio, caption=invoice_text, reply_markup=markup, parse_mode="Markdown")
      except:
          sent_msg = bot.send_photo(chat_id, photo=qr_img_bio, caption=invoice_text, reply_markup=markup)
  ...
  if data == "admin_add_course":
    ...
    bot.edit_message_text("📝 **Step 1/4: प्रोमो मीडिया**\nगैलरी से डेमो फोटो/वीडियो भेजें (एल्बम के लिए एक साथ चुनें)। फिर 'Next Step' दबाएं。", chat_id=chat_id, message_id=msg_id, reply_markup=InlineKeyboardMarkup().row(InlineKeyboardButton("➡️ Next Step", callback_data="next_price")))
  elif data == "admin_create_batch":
    admin_data[ADMIN_ID] = {"mode": "batch", "step": "TITLE", "course_ids": []}
    bot.edit_message_text("📦 **नया कोर्स बैच बनाएँ**\n\nकृपया इस बैच का **नाम/टाइटल** टाइप करके भेजें:", chat_id=chat_id, message_id=msg_id)
    ...
    bot.edit_message_text("💰 **Step 2/4: कीमत**\n\nइस कोर्स की कीमत (₹) भेजें (उदा: `299` या `499`):", chat_id=chat_id, message_id=msg_id)
    ...
    admin_data[ADMIN_ID]["promo"] = []
    admin_data[ADMIN_ID]["caption"] = ""
    bot.edit_message_text("📝 **अगले कोर्स का प्रोमो मीडिया भेजें**\nफिर 'Next Step' दबाएं।", chat_id=chat_id, message_id=msg_id, reply_markup=InlineKeyboardMarkup().row(InlineKeyboardButton("➡️ Next Step", callback_data="next_price")))
    ...
    if not records: bot.edit_message_text("अभी तक किसी ने कोर्स नहीं खरीदा है।", chat_id=chat_id, message_id=msg_id)
      ...
      bot.edit_message_text(text, chat_id=chat_id, message_id=msg_id, reply_markup=InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Back", callback_data="back_to_admin")), parse_mode="Markdown")
    ...
    if course:
      ...
      try:
        ...
        user_name = f"{user_info.first_name} (@{user_info.username})" if user_info.username else f"{user_info.first_name}"
      ...
      with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute("INSERT INTO purchases (user_id, username, item_info, date) VALUES (?, ?, ?, ?)", (user_id, uname, f"{course_id} | Rate: ₹{course['amount']} | APPROVED", date_now))
        conn.commit()
      receipt = f"✅ **[PAYMENT APPROVED & DELIVERED]**\n\n👤 **User:** {user_name}\n🆔 **ID:** `{user_id}`\n📚 **Course:** `{course_id}`\n💰 **Rate:** ₹{course['amount']}\n📅 **Date:** {date_now}"
      if call.message.content_type == "photo": bot.edit_message_caption(receipt, chat_id=chat_id, message_id=msg_id, parse_mode="Markdown")
      else: bot.edit_message_text(receipt, chat_id=chat_id, message_id=msg_id, parse_mode="Markdown")
    ...
    if reason_code == "fake": reason = "आपका स्क्रीनशॉट अमान्य/फेक है।"
    elif reason_code == "notrecv": reason = "हमें आपका पेमेंट प्राप्त नहीं हुआ।"
    elif reason_code == "wrong": reason = "आपने गलत राशि भेजी है।"
    if user_id in pending_verifications:
      try: bot.delete_message(user_id, pending_verifications[user_id]["checking_msg_id"])
      except: pass
      del pending_verifications[user_id]
    ...
    try: bot.send_message(user_id, f"❌ **Payment Denied!**\n\n**कारण:** {reason}", reply_markup=InlineKeyboardMarkup().row(InlineKeyboardButton("🔄 Try Again (QR जनरेट करें)", callback_data=f"pay_upi_{course_id}")), parse_mode="Markdown")
    except: pass
    date_now = datetime.now().strftime("%d-%m-%Y %I:%M:%S %p")
    try:
      user_info = bot.get_chat(user_id)
    ...
    with get_db() as conn:
      cursor = conn.cursor()
      cursor.execute("INSERT INTO purchases (user_id, username, item_info, date) VALUES (?, ?, ?, ?)", (user_id, uname, f"{course_id} | Rate: ₹{rate_amt} | REJECTED: {reason}", date_now))
      conn.commit()
    receipt = f"❌ **[PAYMENT REJECTED]**\n\n👤 **User:** {user_name}\n🆔 **ID:** `{user_id}`\n📚 **Course:** `{course_id}`\n💰 **Rate:** ₹{rate_amt}\n📅 **Date:** {date_now}\n⚠️ **Reason:** {reason}"
    if call.message.content_type == "photo": bot.edit_message_caption(receipt, chat_id=chat_id, message_id=msg_id, parse_mode="Markdown")
    else: bot.edit_message_text(receipt, chat_id=chat_id, message_id=msg_id, parse_mode="Markdown")
