    ...
    if user_id == ADMIN_ID and user_id in admin_data:
        ...
        if step == "BC_MEDIA":
            ...
            if " - " in btn_text:
                ...
                except: bot.send_message(ADMIN_ID, "❌ Format Error.", parse_mode="HTML")
...
def handle_buttons(call):
    ...
    if data == "admin_add_course":
        ...
        if not buttons:
            ...
            return
        markup = InlineKeyboardMarkup()
        for b in buttons: markup.add(InlineKeyboardButton(f"❌ {b['button_text']}", callback_data=f"delmenu_{str(b['_id'])}"))
        ...
        try: menu_buttons_col.delete_one({"_id": ObjectId(btn_id)})
        except: pass
        ...
        if course:
            ...
            purchases_col.insert_one({"user_id": user_id, "username": user_mention, "item_info": f"{course_id} | Rate: ₹{course['amount']} | APPROVED", "date": date_now})
        ...
        purchases_col.insert_one({"user_id": user_id, "username": user_mention, "item_info": f"{course_id} | Rate: ₹{rate_amt} | REJECTED: {reason}", "date": date_now})
...
# ==========================================
# 4. Flask Web Server (Render 24/7 Hosting) & Error Handler 
# ==========================================
app = Flask(__name__)
