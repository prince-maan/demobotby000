...
menu_buttons_col = db["menu_buttons"]
...
def generate_upi_qr(amount, order_id):
  ...
  draw.line(
      [(u_x, start_y + let_h), (u_x + let_w, start_y + let_h)],
      fill="#097939",
      width=lw,
  )
  draw.line(
      [(u_x + let_w, start_y + let_h), (u_x + let_w, start_y)],
      fill="#097939",
      width=lw,
  )
  p_x = u_x + let_w + gap
  ...
  draw.line(
      [(p_x + let_w, start_y), (p_x + let_w, start_y + let_h // 2)],
      fill="#F37021",
      width=lw,
  )
  draw.line(
      [(p_x + let_w, start_y + let_h // 2), (p_x, start_y + let_h // 2)],
      fill="#F37021",
      width=lw,
  )
...
def expire_qr(chat_id, message_id, course_id):
  if (
    ...
    try:
      ...
      bot.send_message(
          chat_id,
          "❌ <b>Your payment session (10 hours) has expired! Please try"
          " again.</b>",
          parse_mode="HTML",
      )
  ...
  if (
    ...
    try:
      bot.edit_message_caption(
          "❌ <b>Auto-Expired (10 Hours)</b> - No Action Taken",
    ...
    try:
      bot.send_message(
          user_id,
          "⏳ <b>Time's up!</b>\nNo approval received from admin in 10 hours."
...
def send_course_to_user(chat_id, course):
  ...
  if len(media_items) == 1:
    ...
    try:
      bot.send_message(
          chat_id,
          "👆 <b>Choose an option to buy this pack"
          f" (₹{course['amount']}):</b>\n",
          reply_markup=markup,
          parse_mode="HTML",
      )
...
def send_batch_to_user(chat_id, batch):
  bot.send_message(
      ...
      "<i>(Niche sabhi packs diye gaye hain:)</i>",
      parse_mode="HTML",
  )
  try:
  ...
  for cid in course_ids:
    ...
    if c_data:
      send_course_to_user(chat_id, c_data)


# 🌟 डायनेमिक स्टार्ट मेन्यू 🌟
def send_main_menu(chat_id):
  buttons = list(menu_buttons_col.find())
  markup = InlineKeyboardMarkup()
  for b in buttons:
    target = b["target_data"]
    if target.startswith("http"):
      markup.row(InlineKeyboardButton(b["button_text"], url=target))
    else:
      markup.row(
          InlineKeyboardButton(
              b["button_text"], callback_data=f"mainmenu_{target}"
          )
      )
...
def send_admin_panel(chat_id):
  markup = InlineKeyboardMarkup()
  markup.row(
      InlineKeyboardButton("➕ Add Single Pack", callback_data="admin_add_course")
  )
  markup.row(
      InlineKeyboardButton(
  ...
  markup.row(
      InlineKeyboardButton(
          "🔗 Advanced File to Link", callback_data="admin_file_link"
      )
  )
  markup.row(
      InlineKeyboardButton(
          "📢 Advanced Broadcast", callback_data="admin_broadcast"
      )
  )
  markup.row(
      InlineKeyboardButton(
          "📋 Manage Main Menu", callback_data="admin_manage_menu"
      )
  )
  markup.row(
...
def start_command(message):
  ...
  if param.startswith("b_"):
    ...
    if batch:
      send_batch_to_user(user_id, batch)
    else:
      bot.send_message(
          user_id,
          "❌ <b>This pack link has expired.</b>",
          parse_mode="HTML",
      )
      ...
      if len(media_items) == 1:
        ...
        for item in media_items:
          if item["type"] == "photo":
            media_group.append(
                InputMediaPhoto(
                    item["file_id"], caption=item["caption"], parse_mode="HTML"
                )
            )
          elif item["type"] == "video":
            media_group.append(
...
def handle_all_messages(message):
  ...
  if user_id == ADMIN_ID and user_id in admin_data:
    ...
    if step == "BC_MEDIA":
      ...
      admin_data[ADMIN_ID].setdefault("media", []).append(
          {"type": media_type, "file_id": file_id, "caption": caption}
      )

      markup = InlineKeyboardMarkup().row(
          InlineKeyboardButton(
              "✅ Done Adding Media/Text", callback_data="bc_done"
          )
      )
      bot.send_message(
          ADMIN_ID,
          "✅ <b>Saved!</b>\nSend another Photo/Video/Text, OR click 'Done' if"
          " finished.",
          reply_markup=markup,
          parse_mode="HTML",
      )
    ...
    # 🌟 2. ADVANCED BROADCAST BUTTONS 🌟
    elif step == "BC_BUTTONS":
      btn_text = message.text.strip()
      if " - " in btn_text:
        try:
          text, url = btn_text.split(" - ", 1)
          admin_data[ADMIN_ID].setdefault("buttons", []).append(
          ...
          markup = InlineKeyboardMarkup().row(
              InlineKeyboardButton(
                  "🚀 Finish & Broadcast", callback_data="bc_finish"
              )
          )
          bot.send_message(
              ADMIN_ID,
              f"✅ <b>Button Added! (Total: {count})</b>\n\nWant to add another"
              " button? Send it in <code>Name - Link</code> format.\n\n<i>(OR"
              " click Finish to start broadcast)</i>",
              reply_markup=markup,
              parse_mode="HTML",
          )
        except Exception:
          bot.send_message(
              ADMIN_ID,
              "❌ Format Error. Use EXACTLY like this: <code>My Website -"
    ...
    # 🌟 3. ADVANCED FILE-TO-LINK MEDIA 🌟
      ...
      markup = InlineKeyboardMarkup().row(
          InlineKeyboardButton(
              "✅ Done Adding Media/Text", callback_data="ftl_done"
          )
      )
      bot.send_message(
          ADMIN_ID,
    ...
    # 🌟 4. ADVANCED FILE-TO-LINK BUTTONS 🌟
      ...
      if " - " in btn_text:
        try:
          ...
          markup = InlineKeyboardMarkup().row(
              InlineKeyboardButton(
                  "🚀 Finish & Create Link", callback_data="ftl_finish"
              )
          )
          bot.send_message(
              ADMIN_ID,
              f"✅ <b>Button Added! (Total: {count})</b>\n\nWant to add another"
              " button? Send it in <code>Name - Link</code> format.\n\n<i>(OR"
              " click Finish to generate link)</i>",
              reply_markup=markup,
              parse_mode="HTML",
          )
        except:
    ...
    # --- Course Creation Flow ---
      ...
      bot.send_message(
          ...
          " <b>Send the promo media (photo/video).</b>",
          parse_mode="HTML",
      ...
      if not clean_amt:
        bot.send_message(
            ADMIN_ID,
            "❌ <b>Please enter the price in numbers only.</b>",
            parse_mode="HTML",
        )
        return
    ...
    # --- Menu Management Flow ---
      ...
      menu_buttons_col.insert_one(
          {"button_text": btn_text, "target_data": target}
      )
      bot.send_message(
          ADMIN_ID,
          f"✅ <b>Button Added!</b>\n\nText: {btn_text}\nLink: {target}",
  ...
  if user_id in user_states and (message.photo or message.text):
    ...
    check_msg = bot.send_message(
        user_id,
        "⏳ <b>Your payment proof is received. It will be approved within 10"
        " minutes...</b>",
        parse_mode="HTML",
    )
    ...
    try:
      if message.photo:
        ...
        admin_msg = bot.send_message(
            DB_CHANNEL_ID, admin_text, reply_markup=markup, parse_mode="HTML"
        )
...
def handle_buttons(call):
  ...
  # 🌟 BROADCAST MEDIA DONE 🌟
  if data == "bc_done":
    admin_data[ADMIN_ID]["step"] = "BC_BUTTONS"
    admin_data[ADMIN_ID]["buttons"] = []
    markup = InlineKeyboardMarkup().row(
        InlineKeyboardButton(
            "🚀 Finish & Broadcast (No Buttons)", callback_data="bc_finish"
        )
    )
    bot.send_message(
        ADMIN_ID,
        "✅ <b>Media/Text Saved!</b>\n\nDo you want to add buttons below the"
        " message?\nSend your FIRST button like this:\n<code>My Website -"
        " https://google.com</code>\n\n<i>(Or click Finish to broadcast"
        " immediately without buttons)</i>",
        reply_markup=markup,
        parse_mode="HTML",
    )
    return
    ...
    for u in users:
      ...
      try:
        ...
        if len(media_items) == 1:
          ...
          if item["type"] == "text":
            bot.send_message(
                uid, item["caption"], reply_markup=markup, parse_mode="HTML"
            )
          elif item["type"] == "photo":
            bot.send_photo(
                uid,
  ...
  if data == "ftl_done":
    ...
    markup = InlineKeyboardMarkup().row(
        InlineKeyboardButton(
            "🚀 Finish & Create Link (No Buttons)", callback_data="ftl_finish"
        )
    )
    bot.send_message(
        ADMIN_ID,
        "✅ <b>Media/Text Saved!</b>\n\nDo you want to add buttons below the"
        " message?\nSend your FIRST button like this:\n<code>My Website -"
        " https://google.com</code>\n\n<i>(Or click Finish to create link"
        " immediately without buttons)</i>",
        reply_markup=markup,
        parse_mode="HTML",
    )
  ...
  # 🌟 FTL FINISH 🌟
    ...
    bot.send_message(
        ADMIN_ID,
        "🎉 <b>File/Album Link Created Successfully!</b>\n\n🔗 Share this"
        f" link:\n<code>{link}</code>",
        parse_mode="HTML",
    )
    del admin_data[ADMIN_ID]
  ...
  if data.startswith("mainmenu_"):
    ...
    if target.startswith("c_"):
      ...
      if course:
        send_course_to_user(chat_id, course)
    elif target.startswith("b_"):
      batch = get_batch_data(target)
      if batch:
        send_batch_to_user(chat_id, batch)
    return
  ...
  if data.startswith("pay_upi_"):
    ...
    if course:
      ...
      invoice_text += (
          "\n👉 <b>Payment Instructions:</b>\n* Please send the <b>Payment"
          " Screenshot</b> here.\n⏳ <i>The payment QR will expire in 10"
          " minutes...!</i>"
      )
      ...
      threading.Timer(
          36000, expire_qr, args=(chat_id, sent_msg.message_id, course_id)
      ).start()
    return
  ...
  if data == "admin_add_course":
    ...
    bot.edit_message_text(
        "📎 <b>Advanced File to Link</b>\nSend your Photos, Videos, Documents,"
        " or Texts (with captions).\n<i>(Apni file/photo/text yahan"
        " bhejein:)</i>",
        chat_id=chat_id,
        message_id=msg_id,
        parse_mode="HTML",
    ...
    bot.edit_message_text(
        "📢 <b>Advanced Broadcast</b>\nSend the Text, Photo, Video, or Document"
        " that you want to broadcast.\n<i>(Apna broadcast message yahan"
        " bhejein:)</i>",
        chat_id=chat_id,
        message_id=msg_id,
        parse_mode="HTML",
    ...
    markup.row(
        InlineKeyboardButton(
            "➕ Add Menu Button", callback_data="admin_menu_add"
        )
    )
    markup.row(
        InlineKeyboardButton(
            "🗑 Delete Menu Button", callback_data="admin_menu_del"
        )
    )
    markup.row(InlineKeyboardButton("🔙 Back", callback_data="back_to_admin"))
    bot.edit_message_text(
        "📋 <b>Manage Main Menu</b>",
        chat_id=chat_id,
        message_id=msg_id,
        reply_markup=markup,
        parse_mode="HTML",
    ...
    bot.edit_message_text(
        "🔗 <b>Add Menu Button</b>\nSend the Course ID (c_...), Batch ID"
        " (b_...), or Website URL (http...):",
        chat_id=chat_id,
        message_id=msg_id,
        parse_mode="HTML",
    )
    ...
    if not buttons:
      bot.edit_message_text(
          "❌ No buttons currently in menu.",
          chat_id=chat_id,
          message_id=msg_id,
          reply_markup=InlineKeyboardMarkup().row(
              InlineKeyboardButton("🔙 Back", callback_data="admin_manage_menu")
          ),
      )
      return
    markup = InlineKeyboardMarkup()
    for b in buttons:
      markup.add(
          InlineKeyboardButton(
              f"❌ {b['button_text']}", callback_data=f"delmenu_{str(b['_id'])}"
          )
      )
    markup.add(
        InlineKeyboardButton("🔙 Back", callback_data="admin_manage_menu")
    ...
    bot.edit_message_text(
        "🗑 <b>Click a button to delete it:</b>",
        chat_id=chat_id,
        message_id=msg_id,
        reply_markup=markup,
        parse_mode="HTML",
    ...
    try:
      menu_buttons_col.delete_one({"_id": ObjectId(btn_id)})
    except:
      pass
    bot.edit_message_text(
        "✅ <b>Button Deleted!</b>",
    ...
    batches_col.update_one(
        {"batch_id": batch_id},
        {"$set": {"batch_id": batch_id, "title": d["title"], "course_ids": c_ids_json}},
        upsert=True,
    )
    ...
    if not records:
      ...
      for r in records:
        text += (
            f"👤 {r.get('username', '')} | 📅 {r.get('date', '')[:10]} | 📚"
            f" <code>{r.get('item_info', '')}</code>\n"
        )
      markup = InlineKeyboardMarkup().row(
    ...
    if course:
      ...
      try:
        ...
        if user_info.username:
          user_mention = f"@{user_info.username}"
        else:
      ...
      purchases_col.insert_one({
          ...
          "item_info": f"{course_id} | Rate: ₹{course['amount']} | APPROVED",
          "date": date_now,
      })

      receipt = (
          "✅ <b>[PAYMENT APPROVED & DELIVERED]</b>\n\n"
  ...
  # --- ADMIN: REASON SELECTED ---
    ...
    if reason_code == "fake":
      reason = "Your screenshot is invalid/fake. Chat now."
    elif reason_code == "notrecv":
      reason = "We haven't received your payment. Chat now."
    elif reason_code == "wrong":
      reason = "You have sent the wrong amount. Chat now."
    ...
    retry_markup = InlineKeyboardMarkup().row(
        InlineKeyboardButton(
            "🔄 Try Again", callback_data=f"pay_upi_{course_id}"
        )
    )
    try:
      bot.send_message(
    ...
    purchases_col.insert_one({
        "user_id": user_id,
        "username": user_mention,
        "item_info": f"{course_id} | Rate: ₹{rate_amt} | REJECTED: {reason}",
        "date": date_now,
    })
...
@app.route("/")
def home():
  return "Telegram Advanced Pack Bot is Running Smoothly on MongoDB 24/7!"
